  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I am your GroundedAI agent. Ask me anything about the documents in this collection.",
    },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${backendUrl}/collections/${collection.id}/chat`,
        {
          method: "POST",
        ...
        <div className="w-80 flex-shrink-0 flex flex-col p-6 gap-6 h-full overflow-y-auto">
          <div className="flex items-center gap-3">
            <Link
              ...
              className="w-10 h-10 rounded-full bg-brand-canvas flex items-center justify-center shadow-sm hover:bg-brand-primary-pale transition-colors"
            >
              <ChevronLeft className="w-5 h-5 text-brand-ink" />
            </Link>
            <h2 className="font-display font-black text-2xl text-brand-ink tracking-tight">
              GroundedAI
            </h2>
          </div>
          ...
          <div className="bg-brand-canvas rounded-xl p-5 shadow-sm flex flex-col gap-4 border-none flex-grow">
            <div>
              <h3 className="text-brand-mute text-xs font-semibold uppercase tracking-wider mb-1">
                Collection
              </h3>
            ...
            <div className="flex flex-col gap-2 flex-grow">
              <h3 className="text-brand-mute text-xs font-semibold uppercase tracking-wider mb-2">
                Documents ({documents.length})
              </h3>
              {documents.map((doc: { id: string; filename: string }) => (
                <div
                  key={doc.id}
                  className="flex items-start gap-3 p-2 hover:bg-brand-canvas-soft rounded-lg cursor-pointer transition-colors"
                >
                  <FileText className="w-5 h-5 text-brand-ink flex-shrink-0 mt-0.5" />
                  <span className="text-sm text-brand-ink font-medium leading-snug truncate">
                    {doc.filename}
                  </span>
                </div>
              ))}
          ...
          <div className="bg-brand-canvas rounded-xl shadow-sm flex flex-col h-full relative overflow-hidden">
            <div className="h-16 border-b border-brand-canvas-soft flex items-center px-6 justify-between flex-shrink-0 z-10 bg-brand-canvas">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-colors ${
                  isTraceOpen
                    ? "bg-brand-primary-pale text-brand-positive-deep"
                    : "bg-brand-canvas-soft text-brand-ink hover:bg-brand-primary-pale"
                }`}
              >
                        ...
                        {msg.groundingScore && (
                          <div
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                              msg.groundingScore > 90
                                ? "bg-brand-positive-pale text-brand-positive-deep"
                                : msg.groundingScore > 70
                                  ? "bg-brand-warning/20 text-brand-warning-deep"
                            ...
                            components={{
                              a: ({ node, href, children, ...props }) => {
                                if (href?.startsWith("#cite-")) {
                                  const sourceIdx =
                                    parseInt(href.replace("#cite-", ""), 10) -
                                    1;
                                  const source =
                                    msg.sources && msg.sources[sourceIdx]
                                      ? msg.sources[sourceIdx]
                                      : null;
                                    ...
                                    <Tooltip key={sourceIdx}>
                                      ...
                                      {source && (
                                        <TooltipContent
                                          side="top"
                                          sideOffset={6}
                                          className="bg-brand-ink text-brand-canvas text-xs px-3 py-2 rounded-xl shadow-xl flex flex-col gap-0.5 w-max max-w-[220px] border-none pointer-events-none"
                                        >
                                          ...
                                          {source.type === "web" &&
                                            source.url && (
                                              <span className="text-brand-canvas/70 font-medium text-[10px] mt-0.5 truncate max-w-full block">
                                                {source.url}
                                              </span>
                                            )}
                                          {source.type === "doc" &&
                                            source.page && (
                                              <span className="text-brand-canvas/70 font-medium text-[10px] mt-0.5">
                                                Page • {source.page}
                                              </span>
                                            )}
                                        </TooltipContent>
                              ...
                              table: ({ node, ...props }) => (
                                <div className="overflow-x-auto my-5 border border-brand-canvas-soft rounded-xl shadow-sm">
                                  <table
                              ...
                              th: ({ node, ...props }) => (
                                <th
                                  className="border-b border-brand-canvas-soft bg-brand-canvas-soft/40 px-4 py-3 font-semibold text-brand-ink"
                                  {...props}
                                />
                              ),
                              td: ({ node, ...props }) => (
                                <td
                                  className="border-b border-brand-canvas-soft px-4 py-3 text-brand-ink"
                                  {...props}
                                />
                              ),
                              p: ({ node, ...props }) => (
                              ...
                              code: ({
                                ...
                                return isInline ? (
                                  <code
                                    className="bg-brand-canvas-soft px-1.5 py-0.5 rounded-md text-brand-ink font-mono text-[13px]"
                                    {...props}
                                  >
                                    {children}
                                  </code>
                                ) : (
                                  <pre className="bg-[#1c1c1c] text-brand-canvas p-4 rounded-xl overflow-x-auto font-mono text-[13px] my-4 shadow-sm border border-[#2a2a2a]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-brand-canvas via-brand-canvas to-transparent pt-12">
              <form
                ...
                className="relative flex items-center bg-brand-canvas-soft rounded-2xl p-2 pr-2 shadow-sm border border-transparent focus-within:border-brand-primary focus-within:bg-brand-canvas transition-all"
              >
                <input
                  type="text"
                  value={query}
              ...
              <div className="text-center mt-3 text-xs text-brand-mute">
                GroundedAI can make mistakes. Always verify the source
                attribution.
              </div>
            </div>
          </div>
          ...
          <div className="w-80 flex-shrink-0 flex flex-col p-6 pl-0 h-full overflow-y-auto transition-all duration-300 transform translate-x-0">
            <div className="bg-brand-ink rounded-xl p-5 shadow-sm flex flex-col h-full overflow-hidden text-brand-primary">
              <div className="flex items-center gap-3 mb-6">
                <Loader2
                  className={`w-5 h-5 text-brand-primary ${isGenerating ? "animate-spin" : ""}`}
                />
                <h3 className="font-display font-bold text-xl text-brand-canvas">
                  Agent Trace
                </h3>
              </div>
              ...
              <div className="flex-grow overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-5">
                ...
                {traces.map((trace, idx) => (
                  <div key={trace.id} className="flex gap-4 relative">
                    ...
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 z-10 border-[3px] border-brand-ink mt-0.5 ${
                    ...
                    >
                      {trace.status === "success" && (
                        <CheckCircle2 className="w-3.5 h-3.5 text-brand-ink" />
                      )}
                      {trace.status === "warning" && (
                        <AlertCircle className="w-3.5 h-3.5 text-brand-ink" />
                      )}
                      {trace.status === "loading" && (
                    ...
                    <div className="flex flex-col pb-4">
                      <span
                        className={`text-sm font-semibold ${trace.status === "loading" ? "text-brand-mute" : "text-brand-canvas"}`}
                      >
                        {trace.step}
                      </span>
                      {trace.time && (
                        <span className="text-brand-primary/70 text-xs mt-0.5">
                          {trace.time}
                        </span>
                      )}
                      {trace.detail && (
